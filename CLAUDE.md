...
/office
  ...
  /sql
    ...
    017_staff_salary_add_reason.sql   ALTER — adds required reason to staff_salary
...
| `staff_salary` | Append-only log of every salary change for a staff member — identical pattern to `staff_work_time_history` (see "Work timing history convention" above), but with **no universal-default fallback**: a staff member with zero rows simply has no resolvable salary. `reason` (added in `017_...sql`) is required going forward — a dropdown preset or free text. `set_by` references the admin who recorded the change. |
| `payouts` | One row per staff member per calendar month (unique on `staff_id` + `month`, stored `'YYYY-MM'`). Generated by `admin/payout/generate.php`; see "Payout" below for the full calculation, the `draft` → `finalized` → `paid` workflow, and `forgiven_amount`/`forgiven_by`/`forgiven_at` (added in `018_...sql`) for waiving a deduction. |
...
- **Salary** (`staff_salary`) is append-only, resolved via
  ...
  Every `staff_salary` row also carries a **required `reason`**
  (`admin/staff/set-salary.php`: a dropdown of presets — Annual
  Increment, Promotion, Performance Adjustment, Market Correction, New
  Hire — Initial Salary, Demotion / Reduction — plus `"Other"`, which
  reveals a free-text field via a plain `onchange` toggle, same inline-JS
  pattern as `set-timing.php`'s custom-hours toggle. Whichever produces
  the final string is stored as-is in one `VARCHAR(255)` column — no
  separate category/note split. Rows from before this column existed
  default to `''`, shown as `"—"` in the Salary History table.
...
- **Formula:**
  ...
  `effective_deduction = effectiveDeduction(unpaid_deduction, forgiven_amount)`
  (`= max(0, unpaid_deduction − forgiven_amount)` — see "Forgiving a
  deduction" below)
  `net_payout = base_salary − effective_deduction + bonus`
  `days_in_month` is the target month's actual calendar length (28-31),
  via `daysInMonth()`.
- **Forgiving a deduction** (`admin/payout/view.php`, `draft` only): an
  admin can waive a payout's entire `unpaid_deduction` with one click
  ("Forgive Deduction") — sets `payouts.forgiven_amount` to the current
  `unpaid_deduction`, `forgiven_by` to the acting admin, `forgiven_at` to
  now, and recomputes `net_payout` using `effectiveDeduction()` (so it
  goes up by the forgiven amount). The payslip shows the original amount
  struck through, a green "Forgiven" badge, and a highlighted box naming
  the admin and timestamp. "Un-forgive" (also draft-only) clears all
  three columns and restores the full deduction. `forgiven_by` is a soft
  reference to `admins.id` — no `FOREIGN KEY`, added via
  `sql/018_payouts_add_forgiveness.sql` (manual "Re-run" required, like
  011/014/016/017) — deliberately skipped a `FOREIGN KEY ... IF NOT
  EXISTS` clause there since that syntax isn't portable between MySQL and
  MariaDB, unlike `ADD COLUMN IF NOT EXISTS`.
  - **Forgiveness is an admin decision, not a derived figure** — exactly
    like `bonus`, it's preserved (not reset) by both "Regenerate" and a
    bulk re-`generate.php` on an existing draft. `effectiveDeduction()`
    lives in `includes/functions.php` (shared by `generate.php` and
    `view.php`) specifically so both recompute paths apply forgiveness
    identically — a bulk regenerate that recomputed `net_payout` without
    it would silently undo an admin's forgiveness the next time the
    month's payouts are bulk-generated.
  - `effectiveDeduction()` is floored at 0, so if a later "Regenerate"
    lowers `unpaid_deduction` below a previously-forgiven amount (e.g.
    corrected attendance data), the effective deduction never goes
    negative — it's just fully covered by the (now oversized) forgiveness.
- **`draft` → `finalized` → `paid` workflow:**
  ...
  - **"Regenerate"** (`view.php`, any status) is the explicit override the
    ...
    fresh via `computePayoutFigures()`, **keeps the existing bonus and
    any forgiven-deduction amount** (both admin decisions, not derived
    figures), resets `status` back to `'draft'`, and clears `paid_at`.
    This is the only way to update a `finalized`/`paid` payout — always a
    single explicit per-payout action,
    never bulk.
...
- **Salary reason + payout deduction forgiveness:** `staff_salary` now
  requires a `reason` on every entry (`admin/staff/set-salary.php`'s
  dropdown-or-"Other" picker); `admin/staff/view.php`'s Salary History
  table gained a Reason column. `admin/payout/view.php` gained
  "Forgive Deduction"/"Un-forgive" (draft-only), which waives/restores a
  payout's `unpaid_deduction` — see "Payout" above for the full
  `effectiveDeduction()` write-up, including why it had to move into
  `includes/functions.php` (shared by both `generate.php`'s bulk upsert
  and `view.php`'s bonus/regenerate recomputes, so none of them silently
  drop a forgiveness the next time they touch the row).
- Verified end-to-end: the salary reason dropdown rejects submission with
  `"Other"` selected and no free text, and stores either the preset or
  the typed text correctly; forgiving a deduction correctly raises
  `net_payout` by the forgiven amount and shows the struck-through
  original amount, a "Forgiven" badge, and the admin's name in a
  highlighted box on the payslip; un-forgiving restores the original
  `net_payout` exactly; both "Regenerate" and a bulk `generate.php` run
  against an already-forgiven draft preserve the forgiveness and recompute
