...
<title>Payout — <?= h($payout['full_name']) ?> — <?= h($payout['month']) ?></title>
  ...
  <div class="card payslip" style="margin-bottom:16px;">
    <div class="toolbar" style="margin-bottom:16px;">
      <div>
        ...
        <p style="margin:0; color:var(--color-muted);">Payslip for <?= h($payout['month']) ?></p>
      </div>
      <span class="badge badge-<?= $payout['status'] === 'paid' ? 'active' : ($payout['status'] === 'finalized' ? 'office_manual' : 'inactive') ?>" style="font-size:0.9rem;"><?= h($statusLabels[$payout['status']] ?? $payout['status']) ?></span>
    </div>
